import { github, jsonRequest, record } from "./github";
...
async function unseal(env: Env, value: string) {
...
export async function ui(request: Request, env: Env): Promise<Response> {
  ...
  if (path === "/" && request.method === "GET")
    return html(
      `<h1>Review labels, ready when a PR opens.</h1><section><p>Install ghfind Review and choose repositories. We create five review-level labels, preserve your existing label settings, and label new pull requests using the author's ghfind score.</p><p>${env.APP_SLUG && env.ENABLED === "true" ? `<a href="https://github.com/apps/${encodeURIComponent(env.APP_SLUG)}/installations/new">Install GitHub App</a>` : "Installation is not open yet."}</p><p>Only pull request write permission is requested. We do not read source files or run pull request code. Disable the old PR review level workflow when switching to this App.</p><p>Current rollout: ${escape(env.ALLOWED_ACCOUNTS === "*" ? "All accounts" : env.ALLOWED_ACCOUNTS)}.</p></section><section><h2>Labels</h2><p>low: 0–39.99 · medium: 40–69.99 · high: 70–89.99 · xhigh: 90–100 · unavailable: score could not be obtained.</p><p>The score describes an author's public GitHub profile; it is not a code review or a merge decision.</p></section>`,
    );
  if (path === "/privacy")
    return html(
      "<h1>Privacy</h1><section><p>GitHub sends installation and pull request events. We verify them and retain only installation/repository IDs, repository names, PR numbers and task status. Author login is sent to ghfind to retrieve a public-profile score. PR text and code are not stored or sent for scoring.</p><p>Completed task records expire after 30 days. Failed task records remain until an operator resolves them. Dashboard sessions expire after one hour; GitHub user tokens are encrypted at rest. Uninstalling the App stops repository access and leaves existing labels in place.</p><p>For deletion requests, contact the maintainers through the source repository.</p></section>",
    );
  if (path === "/login" && request.method === "GET") {
    const installation = url.searchParams.get("installation_id");
    if (!installation || !/^\d{1,16}$/.test(installation) || !env.APP_CLIENT_ID)
