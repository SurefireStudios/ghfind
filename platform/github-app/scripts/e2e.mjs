...
const [mode, repository, slug] = process.argv.slice(2);
if (
  !["prepare", "open", "verify"].includes(mode) ||
  !/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository ?? "")
)
  throw new Error(
    "Usage: node scripts/e2e.mjs prepare|open|verify owner/test-repository [app-slug]",
  );
...
if (mode === "prepare") {
  ...
  const pr = api(`repos/${repository}/pulls/1`);
  const events = api(`repos/${repository}/issues/1/events?per_page=100`);
